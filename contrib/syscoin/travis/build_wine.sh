...
source ./contrib/${name}/travis/electrum_version_env.sh;
...
cd $WINEPREFIX/drive_c/electrum-${name}

rm -rf build
rm -rf dist/electrum-${name}

cp contrib/${name}/deterministic.spec .
cp contrib/${name}/pyi_runtimehook.py .
cp contrib/${name}/pyi_tctl_runtimehook.py .
...
wine pyinstaller -y \
    --name electrum-${name}-$ELECTRUM_VERSION.exe \
    deterministic.spec
...
wine "$NSIS_EXE" /NOCD -V3 \
    /DPRODUCT_VERSION=$ELECTRUM_VERSION \
    /DWINEARCH=$WINEARCH \
    contrib/${name}/electrum.nsi
